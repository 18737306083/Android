...
import com.caiqiufang.utils.DialogUtils;
...
//		this.startActivity(intent);			//扫描成功，跳转到下一个界面
		//MainActivity.scan_content.setText(resultString);	//将扫描得到的结果，传送到首界面的textView中显示出来
		//Toast.makeText(getApplicationContext(),resultString, Toast.LENGTH_LONG).show();
		
		if (resultString.contains("vs")||resultString.contains("win")) {
			//DialogUtils.DialogShow(resultString, MipcaActivityCapture.this);
			
			/**
			 * 二维码扫描的结果包括两部分，比如，congbaoyangrou.22
			 * 然后用正则将其分割成两部分
			 * 其中一部分直接在结果页面上显示座位号，另一部分查找数据库，返回相应配料信息
			 */
			String result[] = resultString.split("\\.");
			System.out.println("**********************************"+resultString+"******************");
			Intent intent = new Intent(MipcaActivityCapture.this,DialogActivity.class);
			Bundle bundle = new Bundle();
			bundle.putString("name", result[0]);//**************
			bundle.putString("placenum", result[1]);
			intent.putExtras(bundle);
			startActivity(intent);
		}else if (resultString.contains("http://")) {
			//扫描拿到结果后，将ip设置为扫描的结果
			Config.SERVER_IP=resultString;
			Config.setSERVER_IP(resultString);
			Toast.makeText(getApplicationContext(), Config.SERVER_IP, Toast.LENGTH_SHORT).show();
			this.finish();
		}else{
			Toast.makeText(MipcaActivityCapture.this, "对不起二维码过时，不能使用", Toast.LENGTH_SHORT).show();
		}
...
//		playBeepSoundAndVibrate();
		...
		if (resultString.equals("")) {
			Toast.makeText(MipcaActivityCapture.this, "对不起，您扫的二维码可能已经失效", Toast.LENGTH_SHORT).show();
		}else {
