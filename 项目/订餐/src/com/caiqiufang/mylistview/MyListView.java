package com.caiqiufang.mylistview;
...
public class MyListView extends ListView
{
	...
	public MyListView(Context context, AttributeSet attrs)
	{
		...
		delete = (Button) view.findViewById(R.id.id_item_btn);		
		update = (Button) view.findViewById(R.id.id_item_btn1);
	...
	public boolean dispatchTouchEvent(MotionEvent ev)
	{
		...
		switch (action)
		{
			...
			case MotionEvent.ACTION_DOWN:
				...
				// 获得当前手指按下时的item
				View view = getChildAt(mCurrentViewPos - getFirstVisiblePosition());	//当前的位置减去第一个可见的位置
				mCurrentView = view;
				break;
			case MotionEvent.ACTION_MOVE:
				...
				if (xMove < xDown && Math.abs(dx) > touchSlop && Math.abs(dy) < touchSlop)
				{
					...
					// " , dy = " + dy);
					isSliding = true;	//是向左滑动
					
				}
				break;
		}
		System.out.println("*********Ev*******************"+ev.getAction()+"**************************");
		return super.dispatchTouchEvent(ev);//***********************1
	}
	...
	public boolean onTouchEvent(MotionEvent ev)
	{
		...
		if (isSliding && mCurrentView != null)
		{
			...
			case MotionEvent.ACTION_MOVE:
				...
				// 获得当前item的位置x与y
				mCurrentView.getLocationOnScreen(location);			//*********************************************1
				...
				update.setOnClickListener(new OnClickListener() {
					...
					@Override
					public void onClick(View v) {
						/*System.out.println("***********************"+"我进来了啊啊啊啊");
						Toast.makeText(getContext(), "************我是update", Toast.LENGTH_LONG).show();*/
						if (mListener != null)
						{	
							mListener.updaHappend(mCurrentViewPos);
							mPopupWindow.dismiss();
						}
					}
				});
